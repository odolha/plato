    ...
    files.forEach(function(file) {
      ...
      if (options.recurse && fs.statSync(file).isDirectory()) {
        ...
        if (options.ecmaOptions && options.ecmaOptions.ignoreDecorators) {
          ...
          var reg = /^(\s\t)*@\w+\(/m;
          var startIndex = source.search(reg);
          var endIndex;
          while (startIndex >= 0) {
            var endIndex = findEndIndex(source, startIndex);
            if (!endIndex) {
              console.error('Problem with removing decorators for source file: ');
              console.error(source);
            }
            source = source.substr(0, startIndex) + source.substr(endIndex + 1);
            startIndex = source.search(reg);
          }
        }
