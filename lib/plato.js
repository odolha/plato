...
function removeDecorators(source, options) {
  // if ignore-decorators, remove all annotations from the source file
  if (options.ecmaOptions && options.ecmaOptions.ignoreDecorators) {
    var psource = source;

    // simple annotations
    psource = psource.replace(/^(\s\t)*@\w+(\s\t)*(;|\n|$)/gm, "");

    // annotations with params, and possible multi-line - brute force approach due to regex complexity
    var findEndIndex = function(psource, startIndex) {
      var indent = 0;
      for (var i=startIndex; i++; false) {
        if (psource[i] === '(') {
          indent++;
        } else if (psource[i] === ')') {
          indent--;
    ...
    var reg = /^(\s\t)*@\w+\(/m;
    var startIndex = psource.search(reg);
    var endIndex;
    while (startIndex >= 0) {
      var endIndex = findEndIndex(psource, startIndex);
      if (!endIndex) {
        console.error('Problem with removing decorators for source file: ');
        console.error(psource);
      }
      psource = psource.substr(0, startIndex) + psource.substr(endIndex + 1);
      startIndex = psource.search(reg);
    }
  }
...
exports.inspect = function(files, outputDir, options, done) {
  ...
  var flags = {
    complexity : {
	  ecmaOptions: options.ecmaOptions,
      logicalor : true,
      switchcase : true,
      forin : false,
      trycatch : false,
        ...
        _.each(reporters,function(reporter, name) {
          ...
          try {
            ...
            if (preProcessors[name]) {
              processedSource = preProcessors[name](source, options);
            }
            report[name] = reporter.process(processedSource || source, flags[name], report.info);
          } catch (e) {
            error = true;
